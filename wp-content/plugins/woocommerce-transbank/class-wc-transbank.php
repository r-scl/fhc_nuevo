    ...
    class WC_Gateway_Transbank extends WC_Payment_Gateway
    {
        ...
        function generate_transbank_payment($order_id)
        {
            ...
            if (isset($result["token_ws"])) {
                ...
                echo "<div class='stgo-alert'><p>El precio del servicio funerario contratado <strong>NO considera el valor de la Cuota Mortuoria</strong></p></div>";

                $url_sitio = site_url();
                echo '<div class="cm-bot"><a href="#" data-toggle="modal" data-target="#modal-cuota"><small>¿Qué es la Cuota Mortuoria (CM) y para qué me sirve?</small></a></div>';
                
                echo "<br/><p>Gracias por su pedido, por favor haga clic en el bot&oacute;n de abajo para pagar con WebPay.</p><br/><br/>";

                return '<form action="' . $url . '" method="post">' .
                        '<input type="hidden" name="token_ws" value="' . $token . '"></input>' .
                        '<input class="bot-webpay" type="submit" value="Pagar con WebPay"></input>' .
                        '</form>';
