	...
	public function getTransactionResult($token){
        ...
        if ($validationResult === TRUE){
			$result = $getTransactionResultResponse->return;
                        update_post_meta( $result->buyOrder, 'codigo_de_autorizacion', $result->detailOutput->authorizationCode );
			if ($this->acknowledgeTransaction($token)){
				...
				if ( ($result->VCI == "TSY" ||  $result->VCI == "A" || $result->VCI == "") && $resultCode == 0){
					$this->logger->writeLog('getTransactionResult', $result->buyOrder, $token, $result->detsailOutput, true);
