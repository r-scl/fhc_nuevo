...
if ( ! class_exists( 'THWCFE_Auto_Update_License' ) ) {
	class THWCFE_Auto_Update_License {
		...
		public function __construct( $file, $software_title, $software_version, $plugin_or_theme, $api_url, $text_domain, $extra ) {
			...
			if ( is_admin() ) {
				...
				$this->thpl_license_setting_page = admin_url('admin.php?page=th_checkout_field_editor_pro&tab=license_settings');
		...
		public function wc_am_deactivate_textarea() {
			...
			echo '/>';
			?><span class="description"><?php _e( 'Deactivates the License Key so you can use it on another blog.', $this->text_domain ); ?></span>
			<?php
		}
		...
		public function create_software_api_url( $args ) {
			$query_string = apply_filters('thwcfe_api_license_query_string', http_build_query( $args, null, '&' ), $args);
